<script>
    $("document").ready(function(){
        ...
        $("#airtimeamount").on("keyup",function(){
           ...
           $(document).ready(function() {
    // Check if cashback checkbox is checked
    if ($("#cashback").is(":checked")) {
        var cashback_value = parseFloat("<?php echo isset($data4->sCashBack) ? $data4->sCashBack : '0'; ?>");
        if (!isNaN(cashback_value)) {
            amounttopay -= cashback_value;
        } else {
   ...
   $("#dataplan").on("change", function(){
    var selected_price = $('#dataplan').find(":selected").attr('dataprice');
    var amounttopay = selected_price;
    
    // Check if cashback checkbox is checked
    if ($("#cashback").is(":checked")) {
        var cashback_value = parseFloat("<?php echo isset($data4->sCashBack) ? $data4->sCashBack : '0'; ?>");
        if (cashback_value) {
            amounttopay = parseFloat(selected_price) - cashback_value;
        }
    }
    $("#amounttopay").val(amounttopay);
});
   ...
   $("#datapinForm").submit(function(e){
    ...
    function verifyNetwork(){
        ...
        else if(/0802|0808|0708|0812|0701|0901|0902|0907|0912|0911/.test(phoneStr))
        {
            verNetwork="AIRTEL";
        }
        ...
        if (selNetwork=="ETISALAT") {
            ...
            {
                ic = "<i class = 'fas fa-exclamation-triangle' style ='color:#B33A3A'></i> <b> Ignore if you sure the <b>numbers correct</b>";
            }
        
        document.getElementById('verifyer').innerHTML = "<b>Network:  "+verNetwork+"  "+ic+"</b> ";
    }}
 ...
 function calculateDollarToPay() {
    let rate = <?php echo isset($rate) ? $rate : "0"; ?>;
    let charges = <?php echo isset($charges) ? $charges : "0"; ?>;
    let amount = parseFloat($("#amount").val());

    charges = parseFloat(charges);

    let totalAmount = amount + charges;
    let amountToPay = totalAmount * rate;

    $("#amountToPay").text("Amount to Pay: N" + amountToPay);
    $("#amounttopay").val(amountToPay);
}
